...
<body>
    ...
    <div class="max-w-7xl mx-auto">
        <h1 class="text-[20px] font-bold text-center mt-[20px]">Quản lý danh mục</h1>
        <?php
        require_once('../db.php');
        $sql = "SELECT * FROM danh_muc";
        $statement = $connect->prepare($sql);
        $statement->execute();
        $danh_muc = $statement->fetchAll();
        ?>
        <a href="add.php"><button class="border-2 p-3 font-bold rounded-2xl mb-[20px]">Thêm sản phẩm mới</button></a>
            ...
            <tbody>
                <?php foreach ($danh_muc as $key => $value) : ?>
                    <tr class="p-5">
                        ...
                        <td class="p-5">
                            <a href="form_update.php?id=<?= $value['id'] ?>" class="border-2 p-2 mr-[10px] font-bold rounded-2xl">
                                <button>Sửa danh mục</button>
                            </a>
                            <a href="delete.php?id=<?= $value['id'] ?>" class="border-2 p-2 font-bold rounded-2xl">
                                <button onclick="return confirm('Ban co muon xoa khong?')">Xóa danh mục</button>
                            </a>
                        </td>
                    </tr>
                <?php endforeach ?>
